...
const Login = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#020617] font-['Outfit'] relative overflow-hidden p-6 dark">
      ...
      {/* Main Card */}
      <div className="w-full max-w-[480px] glass-card p-6 sm:p-10 rounded-[2.5rem] shadow-[0_50px_100px_rgba(0,0,0,0.5)] relative z-10 animate-fade-in-up border border-white/20">
        ...
        <div 
          ...
          onClick={() => navigate('/')}
        >
          <div className="w-8 h-8 bg-white rounded-xl flex items-center justify-center group-hover:rotate-[10deg] transition-all duration-500 shadow-[0_0_30px_rgba(255,255,255,0.2)]">
            <div className="w-4 h-4 bg-[#0D0066] rounded-md animate-pulse"></div>
          </div>
        ...
        {errorMsg && (
          <div className="bg-red-500/10 border border-red-500/20 backdrop-blur-xl text-red-400 font-bold text-xs p-3 rounded-xl mb-4 text-center uppercase tracking-widest animate-shake">
            {errorMsg}
          </div>
        )}
        {successMsg && (
          <div className="bg-green-500/10 border border-green-500/20 backdrop-blur-xl text-green-400 font-bold text-xs p-3 rounded-xl mb-4 text-center uppercase tracking-widest animate-fade-in">
            {successMsg}
          </div>
        )}
        ...
        {isForgotPassword ? (
          <>
            <div className="text-center mb-6">
              <h2 className="text-2xl font-black text-white mb-2">{t('forgot_title')}</h2>
              <p className="text-gray-400 font-bold text-xs tracking-tight italic">{t('forgot_subtitle')}</p>
            </div>
            <form onSubmit={handleForgotPassword} className="flex flex-col gap-4">
              <div className="relative group">
                <input 
                  ...
                  required
                  className="w-full h-[50px] pl-5 pr-4 bg-white/5 border border-white/10 rounded-xl text-white placeholder:text-white/20 focus:outline-none focus:bg-white/10 focus:border-[#2F00E6]/50 transition-all font-bold duration-500 text-sm"
                />
              </div>
              <button 
                type="submit"
                disabled={loading}
                className="group relative w-full bg-[#2F00E6] text-white font-black text-xs uppercase tracking-[0.2em] py-4 rounded-xl shadow-[0_20px_50px_rgba(47,0,230,0.3)] hover:bg-[#1200AB] active:scale-95 transition-all duration-500 overflow-hidden"
              >
          ...
          <>
            <div className="text-center mb-6">
              <h2 className="text-2xl font-black text-white mb-2">{isSignUp ? t('signup_welcome') : t('login_welcome')}</h2>
              <p className="text-gray-400 font-bold text-xs tracking-tight italic">{isSignUp ? t('signup_subtitle') : t('login_subtitle')}</p>
            </div>
            ...
            <form onSubmit={handleAuth} className="flex flex-col gap-4">
              <div className="relative group">
                <input 
                  ...
                  onChange={(e) => setEmail(e.target.value)}
                  required
                  className="w-full h-[50px] pl-5 pr-4 bg-white/5 border border-white/10 rounded-xl text-white placeholder:text-white/20 focus:outline-none focus:bg-white/10 focus:border-[#2F00E6]/50 transition-all font-black text-sm tracking-widest"
                />
              </div>
              ...
              <div className="relative group">
                <input 
                  ...
                  required
                  className="w-full h-[50px] pl-5 pr-12 bg-white/5 border border-white/10 rounded-xl text-white placeholder:text-white/20 focus:outline-none focus:bg-white/10 focus:border-[#2F00E6]/50 transition-all font-black text-sm tracking-widest"
                />
                <button
                  type="button"
              ...
              <div className="flex justify-center my-2 min-h-[65px] transform scale-90 sm:scale-100 origin-center">
              ...
              <button 
                type="submit"
                disabled={loading}
                className={`group relative w-full ${loading ? 'bg-gray-700 cursor-not-allowed opacity-50' : 'bg-[#2F00E6] hover:bg-[#1200AB]'} text-white font-black text-xs uppercase tracking-[0.2em] py-4 rounded-xl shadow-[0_20px_50px_rgba(47,0,230,0.3)] active:scale-95 transition-all duration-500 overflow-hidden`}
              >
                <div className="absolute inset-0 -translate-x-full bg-gradient-to-r from-transparent via-white/20 to-transparent group-hover:animate-[shimmer_1.5s_infinite]"></div>
            ...
            <div className="flex gap-3">
              <button 
                ...
                disabled={loading}
                className="flex-1 bg-white hover:scale-105 active:scale-95 text-[#0D0066] font-black text-[10px] uppercase tracking-widest py-3 px-2 rounded-xl shadow-xl transition-all duration-300 flex items-center justify-center gap-2"
              >
              ...
              <button 
                ...
                disabled={loading}
                className="flex-1 bg-[#24292e] hover:scale-105 active:scale-95 text-white font-black text-[10px] uppercase tracking-widest py-3 px-2 rounded-xl shadow-xl transition-all duration-300 flex items-center justify-center gap-2 border border-white/10"
              >
